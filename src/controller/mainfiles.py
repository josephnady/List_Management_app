import time
from src.model.Deactivator import Deactivate, Deactivate_sync
from src.model.Activator import Activate, Activate_listmanagment, Activate_Pool_Phase1
from src.service.data_collector_service import DataCollectorService as dcs
from src.utils.utils import Utils
from multiprocessing import Pool
from src.view.deactiv_gui import DeactGUI as d
from selenium.webdriver.common.by import By


def main_Deactivate():
    Deactive_list = dcs().deactivated_ids_collector()
...
def main_Activate(c):
    ...
    ActivationIDs = dcs().activation_ids_collector()
...
def main_Activate_sync(activelist):
    ...
    browser = Utils.open_refresh_without_close("http://newcrm.liptispharma.com:88/liptis/crm/index.php")
    ...
    if ActivationIDs2:
        for i, newID in enumerate(ActivationIDs2, 1):
            try:
                ...
                Utils.navigate_to(browser, newID)
...
def main_Act_Deact(c):
    ...
    results = dcs().act_deact_ids_collector()
    # print(len(results))
    DeactIDs = results[0]
    ActIDs = results[1]
