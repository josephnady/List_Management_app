...
def Deactivate(FinalInActiveListNoDuplicate,lininput,terinput):
    # variables
    ...
    print(f"length of list \"Active\" are {len(CrmIdList)}")
    ...
    # Save the report
    u.save_report(FinalInActiveListNoDuplicate,TerrInputvar,2)




def Deactivate_sync(FinalInActiveListNoDuplicate,lin,terr):
    print(f"{' Start Deactivation ':*^40}")
    lininput = lin
    terinput = terr
    # variables
    LineInputvar = str(lininput).strip()
    ...
    print(f"length of \"Active\" list are {len(CrmIdList)}")
